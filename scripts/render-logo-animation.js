...
const TAIL_MS = parseInt(process.env.TAIL_MS || '250', 10);   // beat of empty after fade
...
(async () => {
  ...
  await page.evaluate((holdMs) => {
    ...
    document.getAnimations().forEach(a => {
      a.pause();
      const name = a.animationName || (a.effect && a.effect.getKeyframes && '');
  ...
  // ── Phase 3: a short empty tail so the sting can breathe ────────────────
  await page.evaluate(() => {
    document.getAnimations().forEach(a => {
